	...
	const idEstagiario = localStorage.getItem("idEstagiario");
	
	async function carregarTarefas() {
	
	    if (!idEstagiario) {
	        alert("Estagiário não identificado.");
	        return;
	    }
	
	    const response = await fetch(API_TAREFAS_ESTAGIARIO + idEstagiario);
	                    ...
	                    <div class="detail-grid">
	                        ...
	                        <div class="detail-card submission-area">
	                            ...
	                            <label class="drop-zone">
									...
									<button
									    class="btn-submit"
									    onclick="enviarTarefa(${item.id}, ${item.tarefasEntity.id})">
	...
	function concluirTarefa(id){
	    ...
	    alert("Implementar envio da atividade da tarefa " + id);
	...
	async function enviarTarefa(idTarefaUsuario, idTarefa){
	    ...
	    const arquivo = document.getElementById("arquivo-" + idTarefaUsuario).files[0];
	    const observacoes = document.getElementById("obs-" + idTarefaUsuario).value;

		console.log("idTarefaUsuario:", idTarefaUsuario);
